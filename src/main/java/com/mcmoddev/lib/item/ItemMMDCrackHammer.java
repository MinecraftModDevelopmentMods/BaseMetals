...
public class ItemMMDCrackHammer extends net.minecraft.item.ItemTool implements IMMDObject {
	...
	public ItemMMDCrackHammer(final MMDMaterial material) {
		super(1 + Materials.getToolMaterialFor(material).getDamageVsEntity(), ATTACK_SPEED,
				Materials.getToolMaterialFor(material), new HashSet<Block>());
		this.material = material;
		this.damageVsEntity = 5F + (2F * this.material.getBaseAttackDamage());
	...
	public boolean onBlockDestroyed(final ItemStack tool, final World world,
		...
		if (!world.isRemote && this.canHarvestBlock(target)) {
			...
			if (recipe != null) {
				...
				if (output != null) {
					final int num = output.stackSize;
					output.stackSize = 1;
					for (int i = 0; i < num; i++) {
	...
	@Override
	public EnumActionResult onItemUse(final ItemStack item, final EntityPlayer player,
			final World w, final BlockPos coord, final EnumHand hand, final EnumFacing facing,
		...
		for (final EntityItem target : entities) {
			final ItemStack targetItem = target.getEntityItem();
			if (targetItem != null) {
				final ICrusherRecipe recipe = CrusherRecipeRegistry.getInstance()
						.getRecipeForInputItem(targetItem);
				if (recipe != null) {
					// hardness check
					if ((Options.enforceHardness())
							&& (targetItem.getItem() instanceof ItemBlock)) {
					...
					if (!w.isRemote) {
						final ItemStack output = recipe.getOutput().copy();
						final int count = output.stackSize;
						int toolDamage;
	...
	protected ICrusherRecipe getCrusherRecipe(final IBlockState blockState) {
		...
		return CrusherRecipeRegistry.getInstance().getRecipeForInputItem(
	...
	public boolean canHarvestBlock(final IBlockState targetBS) {
		...
		if (this.toolTypes.contains(toolType) || (target.getMaterial(targetBS) == Material.ROCK)) {
			...
			return this.getHarvestLevel(null, Names.PICKAXE.toString()) >= target
					.getHarvestLevel(target.getDefaultState());
