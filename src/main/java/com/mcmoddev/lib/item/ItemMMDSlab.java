...
public class ItemMMDSlab extends net.minecraft.item.ItemSlab implements IMMDObject {
	 ...
	 */
	@Override
	public EnumActionResult onItemUse(final ItemStack itemstack, final EntityPlayer player, final World worldIn, final BlockPos pos, final EnumHand hand, final EnumFacing facing, final float hitX, final float hitY, final float hitZ) {
		if ((itemstack.stackSize != 0) && player.canPlayerEdit(pos.offset(facing), facing, itemstack)) {
			...
			return this.tryPlace(player, itemstack, worldIn, pos.offset(facing)) ? EnumActionResult.SUCCESS : super.onItemUse(itemstack, player, worldIn, pos, hand, facing, hitX, hitY, hitZ);
		} else {
