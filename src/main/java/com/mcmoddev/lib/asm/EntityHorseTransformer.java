			...
			if (ITransformer.oneEquals(ENTITY_INIT, name)) {
				return new MethodVisitor(Opcodes.ASM5, mv) {
					...
					public void visitInsn(int opcode) {
						if (opcode == Opcodes.RETURN){
							...
							mv.visitMethodInsn(INVOKESTATIC, ASMHOOKS_INTERFACE, "onInitHorse", "(Lnet/minecraft/entity/passive/EntityHorse;)V", false);
						}
						mv.visitInsn(opcode);
					}
				};
				...
				return new MethodVisitor(Opcodes.ASM5, mv) {
					...
					public void visitCode() {
						...
						mv.visitMethodInsn(INVOKESTATIC, ASMHOOKS_INTERFACE, "setHorseArmorStack", "(Lnet/minecraft/entity/passive/EntityHorse;Lnet/minecraft/item/ItemStack;)V", false);
					}
				};
			} else if (ITransformer.oneEquals(SET_HORSE_TEXTURE_PATHS, name)) {
				return new MethodVisitor(Opcodes.ASM5, mv) {
					@Override
					public void visitMethodInsn(int opcode, String owner, String methodCallName, String desc, boolean itf) {
						if (ITransformer.oneEquals(GET_TEXTURE_NAME, methodCallName)){
							mv.visitVarInsn(ALOAD, 0);
							mv.visitMethodInsn(INVOKESTATIC, ASMHOOKS_INTERFACE, "getTextureName", "(Lnet/minecraft/entity/passive/HorseArmorType;Lnet/minecraft/entity/passive/EntityHorse;)Ljava/lang/String;", false);
