...
//						material.getItemStack(Names.INGOT, 13), 0); // You lose half an ingot
	...
	protected static void furnaceSpecial(@Nonnull final MMDMaterial material) {
		...
		if (material.hasItem(Names.INGOT)) {
			...
			if (Options.furnaceCheese()) {
				...
				final ItemStack outputStack = material.getItemStack(Names.NUGGET, 1);

				final Names[] itemNames = { Names.BOOTS, Names.HELMET, Names.SWORD, Names.SHOVEL,
						Names.PICKAXE, Names.HOE, Names.AXE, Names.LEGGINGS, Names.CHESTPLATE,
						Names.CRACKHAMMER };
				for (final Names itemName : itemNames) {
	...
	protected static void addAdditionalOredicts(@Nonnull final MMDMaterial material,
		...
		if (material.hasOre()) {
			registerBlockOreDict(material, Names.ORE, oreDictName);
		}
		registerBlockOreDict(material, Names.BLOCK, oreDictName);
		registerBlockOreDict(material, Names.PLATE, oreDictName);
		registerBlockOreDict(material, Names.BARS, oreDictName);
		registerBlockOreDict(material, Names.DOOR, oreDictName);
		registerBlockOreDict(material, Names.TRAPDOOR, oreDictName);
		if (material.hasBlend()) {
			registerItemOreDict(material, Names.BLEND, oreDictName);
			if (material.hasItem(Names.SMALLBLEND)) {
				registerItemOreDict(material, Names.SMALLBLEND, oreDictName);
		...
		if (material.hasItem(Names.INGOT)) {
			if (material.getType() != MaterialType.GEM) {
				OreDictionary.registerOre(Oredicts.INGOT + oreDictName,
						material.getItemStack(Names.INGOT)); // For non-gem based
		...
		registerItemOreDict(material, Names.NUGGET, oreDictName);
		registerItemOreDict(material, Names.POWDER, oreDictName);
		if (material.hasItem(Names.SMALLPOWDER)) {
			registerItemOreDict(material, Names.SMALLPOWDER, oreDictName);
			OreDictionary.registerOre(Oredicts.DUST_SMALL + oreDictName,
					material.getItemStack(Names.SMALLPOWDER));
		}
		registerItemOreDict(material, Names.GEAR, oreDictName);
		if (material.hasItem(Names.ROD)) {
			registerItemOreDict(material, Names.ROD, oreDictName);
			OreDictionary.registerOre(Oredicts.STICK + oreDictName,
					material.getItemStack(Names.ROD));
		}
	}

	protected static void registerBlockOreDict(MMDMaterial material, Names thingName) {
		registerBlockOreDict(material, thingName, material.getName());
	}

	protected static void registerBlockOreDict(MMDMaterial material, Names thingName, String materialName) {
		final String oreDictBase = Blocks.getOredictFromName(thingName);
		if ((oreDictBase != null) && (material.hasBlock(thingName))) {
			OreDictionary.registerOre(oreDictBase + materialName,
					material.getBlockItemStack(thingName));
		}
	}

	protected static void registerItemOreDict(MMDMaterial material, Names thingName) {
		registerItemOreDict(material, thingName, material.getName());
	}

	protected static void registerItemOreDict(MMDMaterial material, Names thingName, String materialName) {
		final String oreDictBase = Items.getOredictFromName(thingName);
		if ((oreDictBase != null) && (material.hasItem(thingName))) {
			OreDictionary.registerOre(oreDictBase + materialName,
