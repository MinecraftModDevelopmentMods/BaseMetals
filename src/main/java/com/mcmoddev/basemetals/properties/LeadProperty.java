...
public class LeadProperty extends BMEPropertyBase {
	@Override
	...
	public void apply(ItemStack stack, EntityPlayer player) {
		if(hasEffect(stack, player)) {
			final int level = countArmorPieces(Materials.getMaterialByName(MaterialNames.LEAD), player) / 2;
			if (level == 0) {
				return;
			}
			if (level > 0) {
	...
	public boolean hasEffect(ItemStack stack, EntityPlayer player) {
		...
		boolean rv = (stackIsArmorMaterial(stack, mat) 
				...
				(countArmorPieces(Materials.getMaterialByName(MaterialNames.LEAD),player) > 0);
