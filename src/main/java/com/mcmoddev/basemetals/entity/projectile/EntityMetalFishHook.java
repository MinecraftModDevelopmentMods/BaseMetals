package com.mcmoddev.basemetals.entity.projectile;

import java.util.List;

import com.mcmoddev.lib.items.ItemMetalFishingRod;
		...
		if (this.fishPosRotationIncrements > 0) {
			...
			if (!this.inGround) {
				...
				for (int l = 0; l < 5; ++l) {
					...
					final double d10 = axisalignedbb.minY + ((d9 * (double) l) / 5.0D);
				...
				if (!this.world.isRemote && (d5 > 0.0D)) {
					...
					if (this.ticksCatchable > 0) {
						...
						if (this.ticksCatchableDelay <= 0) {
							...
							final double d15 = (double) ((float) MathHelper.floor(this.getEntityBoundingBox().minY) + 1.0F);
						...
						if (this.rand.nextFloat() < f4) {
							...
							final double d14 = (double) ((float) MathHelper.floor(this.getEntityBoundingBox().minY) + 1.0F);
