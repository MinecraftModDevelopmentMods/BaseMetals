		...
		if (this.fishPosRotationIncrements > 0) {
			...
			if (!this.inGround) {
				...
				for (int l = 0; l < k; ++l) {
					...
					final AxisAlignedBB axisalignedbb2 = new AxisAlignedBB(axisalignedbb.minX, d10, axisalignedbb.minZ,
							axisalignedbb.maxX, d11, axisalignedbb.maxZ);
		...
		if (this.worldObj.isRemote)
			...
			if (this.caughtEntity != null) {
				...
				for (final ItemStack itemstack : this.worldObj.getLootTableManager().getLootTableFromLocation(LootTableList.GAMEPLAY_FISHING).generateLootForPools(this.rand, lootcontext.build())) {
					final EntityItem entityitem = new EntityItem(this.worldObj, this.posX, this.posY, this.posZ, itemstack);
